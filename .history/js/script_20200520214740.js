...
const loadNews = async(page) => {
    ...
    let url;
    if (!sources.length)
        url = `https://newsapi.org/v2/top-headlines?apiKey=${apiKey}&page=${page}&pageSize=${pageSize}`;
    else {
        let sourceStr = sources.join(",");
        alert(sourceStr);
        url = `https://newsapi.org/v2/top-headlines?page=${page}&pageSize=${pageSize}&sources=${sourceStr}&apiKey=${apiKey}`
    }
...
async function countCategory() {
    let filterUrl = `https://newsapi.org/v2/top-headlines?apiKey=${apiKey}`;
