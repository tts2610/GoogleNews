...
const loadNews = async(page) => {
    ...
    if (!sources.length)
        url = `https://newsapi.org/v2/everything?q=us&apiKey=${apiKey}&page=${page}&pageSize=${pageSize}`;
    else {
        let sourceStr = sources.join(",");
        alert(sourceStr);
        url = `https://newsapi.org/v2/everything?page=${page}&pageSize=${pageSize}&sources=${sourceStr}&apiKey=${apiKey}`
    }
...
function countCategory() {
    let url = `https://newsapi.org/v2/everything?sources=${sourceStr}&apiKey=${apiKey}`;
    let data = await fetch(url);
    let result = await data.json();
...
function reloadFilter() {
    ...
    categoryList.forEach(element => {
        let html = `<div class="form-check">
                        <input class="form-check-input" onchange="loadNews(1);" type="checkbox" id="${element.id}" value="${element.category}">
