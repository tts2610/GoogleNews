...
const loadNews = async(page) => {
    ...
    if (!sources.length)
        ...
        alert(sourceStr);
        url = `https://newsapi.org/v2/everything?q=${q}&page=${page}&pageSize=${pageSize}&sources=${sourceStr}&apiKey=${apiKey}`
    }
...
async function countCategory() {
    let filterUrl = `https://newsapi.org/v2/everything?q=${q}&apiKey=${apiKey}`;
    let data = await fetch(filterUrl);
    let result = await data.json();
