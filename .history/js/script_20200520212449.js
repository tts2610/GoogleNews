...
const loadNews = async(page) => {
    ...
    if (!sources.length)
        ...
        alert(sourceStr);
        url = `https://newsapi.org/v2/everything?q=${us}&page=${page}&pageSize=${pageSize}&sources=${sourceStr}&apiKey=${apiKey}`
    }
...
async function countCategory() {
    let url = `https://newsapi.org/v2/everything?q=apiKey=${apiKey}`;
    let data = await fetch(url);
    let result = await data.json();
