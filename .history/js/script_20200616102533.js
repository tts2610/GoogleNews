...
const loadNews = async (page, category) => {
  ...
  $("#myContent").append(
    `<button onclick="loadMoreFunction('${category}')" id="loadMoreBtn" type="button" class="btn btn-success my-4">Load more</button>`
  );
  $("#myContent").append(
    `<div id="currentNews" style="text-align:right;">Shown : ${
      $("#myContent .row").length
    } news</div>`
  );
};
...
function filterBySource(elem) {
  ...
  let innerHtml = newFilters
    .map((x) => {
      return `<div class="row">
        <div class="news-img"><img class="rounded" src="${
          x.urlToImage
        ...
        <div class="blog-entry-left">
            <div class="text">
                ...
                <div class="meta-wrap">
                    <p class="meta">
                        <span><i class="icon-calendar mr-2"></i>${moment(
                          x.publishedAt
                        ).fromNow()}</span>
                        <span><a href="single.html" id="${
                          x.source.id
    ...
    .join("");
  if (!$("#myContent").find("#filtering").length) $("#myContent").empty();
  if (isAnyChecked()) {
...
function addToDictionary(x) {
  ...
  if (!flag) sourceList.push({ id: x.id, category: x.name, count: 1 });
}

function render(result) {
  let innerHtml = result.articles
    .map((x) => {
